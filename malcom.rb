...
# Introduction
# In the previous lesson, we discussed the concept of inheritance. We learned that through subclasses (a certain class under a super class), the subclass has access to all of the methods of its parent class. Inheriting one class from another makes sense. The subclass can be understood as a child or subordinate of the super class. For example, a car is a type of vehicle, so it makes sense for the Car class to inherit from the Vehicle class.

# Let's think about a slightly different type of example, one that is less hierarchical. We could easily envision writing an app that models the environment of a dance performance. Such an app might have a Dancer class. Dancers, we know, perform dances. Similarly, we could imagine a little kid going to see the Nutcracker ballet one Christmas, coming home and wanting to practice all of the ballet moves from the show. So, we might write a Kid class in which an instance of that class, our little kid who has gone to see the ballet, should have access to all those ballet moves (their performance skill notwithstanding).

# This situation is not hierarchical, like our Car and Vehicle example. Instead, Kid and Dancer simply need to share some functionality, without being related in any other meaningful way.
...
As you can see, we were able to include the #twirl instance method with our Kid instance, and the #take_a_bow method with our Dancer instance, even though those methods weren't defined directly in the classes! This is the benefit of using modules: we can share behavior between multiple classes without relying on inheritance.
...
Open up the lib/meta_dancing_module.rb and define the following module and methods:
...
Because we included the FancyDance::InstanceMethods nested module, we can call those instance methods on instances of our classes.
...
#   end
# end
# The reason for this namespacing is to avoid variable collision: what if our application also happened to have a class definition for Base or Migration? Thanks to namespacing, we can tell that these class definitions are specific to the ActiveRecord module.
